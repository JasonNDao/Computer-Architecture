    ...
    int main(int argc, char *argv[])
{
    ...
    char vars[num + outnum][10]; //names of all input variables
    twopower = pow(2, num);
    for (int i = 0; i < num; i++)
    {
        fscanf(pointer, "%s\t", vars[i]);
    }
    ...
    int **truthtable = (int **)malloc(twopower*sizeof(int *));
    for (int i = 0; i < twopower; i++)
    {
        truthtable[i] = (int *)malloc((num + outnum) * sizeof(int));
    }
    ...
    while (fscanf(pointer, "%s\t", temp) == 1)
    {
        if (strcmp(temp, "NOT") != 0)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                {
                    if (strcmp(tempstuff[i], vars[j]) == 0)
                    {
                        if (i == 0)
                        {
                            one = j;
